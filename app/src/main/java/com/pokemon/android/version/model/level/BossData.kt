...
class BossData(var id: Int, var level: Int, var hpMultiplier: Int, var moves: List<Move>, var boostedStats: List<Stats>, var itemHeld: HoldItem?) {
    companion object {
        fun of(bossEntity: BossEntity, gameDataService: GameDataService): BossData {
            return BossData(bossEntity.id, bossEntity.level, bossEntity.hpMultiplier,
                bossEntity.moveIds.map { gameDataService.getMoveById(it) },
                bossEntity.boostedStats.map { Stats.valueOf(it) },
                if (bossEntity.itemHeld == null || bossEntity.itemHeld == "") null else HoldItem.valueOf(bossEntity.itemHeld))
        }
    }
}