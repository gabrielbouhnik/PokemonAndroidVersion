...
class PokemonBanner(var pokemonData: PokemonData,
    ...
    companion object{
        fun of(pokemonBannerEntity : PokemonBannerEntity, gameDataService: GameDataService): PokemonBanner {
            ...
            val move2 = if (pokemonBannerEntity.moveIds.size > 1) gameDataService.getMoveById(pokemonBannerEntity.moveIds[1]) else null
            val move3 = if (pokemonBannerEntity.moveIds.size > 2) gameDataService.getMoveById(pokemonBannerEntity.moveIds[2]) else null
            val move4 = if (pokemonBannerEntity.moveIds.size > 3) gameDataService.getMoveById(pokemonBannerEntity.moveIds[3]) else null
