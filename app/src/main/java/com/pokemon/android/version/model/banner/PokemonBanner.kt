...
class PokemonBanner(
    ...
    companion object {
        fun of(pokemonBannerEntity: PokemonBannerEntity, gameDataService: GameDataService): PokemonBanner {
            return PokemonBanner(
                pokemonBannerEntity.id,
                gameDataService.getPokemonDataById(pokemonBannerEntity.id).name,
                pokemonBannerEntity.moveIds.map { gameDataService.getMoveById(it) },
                pokemonBannerEntity.rate
            )
        }
    }
